...
class DataService : Service() {
    ...
    companion object {
        ...
        fun getAllInstalledApps() {
            if (!gettingAllInstalledApps.get()) {
                ...
                try {
                    ...
                    // GET_META_DATA ensures applicationInfo is populated on all API levels.
                    // QUERY_ALL_PACKAGES permission (declared in manifest) is required on
                    // Android 11+ (API 30) for getInstalledPackages to return all packages.
                    val packs: ArrayList<PackageInfo> =
                        pm.getInstalledPackages(PackageManager.GET_META_DATA) as ArrayList<PackageInfo>
                    ...
                    Toast.makeText(
                        App.get(), App.get().getString(R.string.cannot_get_all_apps),
                        Toast.LENGTH_LONG
                    ).show()
                ...
                Log.d(
                    TAG, "getAllInstalledApps END, duration = " +
        ...
        fun getAppsByFilter(widgetId: Int, button: WidgetActions?) {
            ...
            if (button != WidgetActions.TEXTFIELD_BUTTON ||
                ...
                if (PInfoHandler.pInfosNotExist()) {
                    Thread { getAllInstalledApps() }.start()
                    return
                } else {
            ...
            Log.d(
                TAG, "getAppsByFilter end " + Calendar.getInstance().timeInMillis + ", " +
                        (Calendar.getInstance().timeInMillis - start)
            )
        }
        ...
        fun getMyWidgets(): HashMap<Class<*>, IntArray> {
            ...
            for (pi in appWidgetProviderInfoList.filter { awpi -> awpi.provider.packageName == App.get().packageName }) {
                ...
                try {
                    ...
                    widgetIds =
                        appWidgetManager.getAppWidgetIds(ComponentName(App.get(), widgetClass))
                    Log.d(TAG, "appWIdgetIds length: " + widgetIds.size)
                    if (widgetIds != null && widgetIds.isNotEmpty()) widgetMap[widgetClass] =
                        widgetIds
        ...
        fun updateWiddgetsOfClassOnAction(
            widgetClass: Class<*>,
            widgetIds: IntArray,
...
// since it seems the onUpdate() is only fired on that:
            ...
            Log.d(
                TAG,
                "updateWiddgetsOfClassOnAction: widgetClass: ${widgetClass.name} widgetIds: ${widgetIds.contentToString()}"
            )
